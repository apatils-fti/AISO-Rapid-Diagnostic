/**
 * Claude Sonnet 4.6 Service
 *
 * Handles:
 * - API calls to Claude API (via server-side route)
 * - Response parsing
...
export class ClaudeService {
  private static STORAGE_KEY = 'claude-results';
  private static COUNTER_KEY = 'claude-request-count';

  /**
   * Check Claude response for a prompt
   */
  static async checkPrompt(
    ...
    if (existing) {
      console.log('[Claude] Using cached result for prompt:', promptId);
      return existing;
    }

    // Check request limit
    const requestCount = this.getRequestsUsed();
    if (requestCount >= CLAUDE_CONFIG.MAX_REQUESTS) {
      throw new Error(
        `Request limit reached (${CLAUDE_CONFIG.MAX_REQUESTS}). Cannot make more queries.`
      );
    }

    console.log('[Claude] Querying Claude Sonnet 4.6 for:', promptText);

    try {
      // Call API route (handles rate limiting and API key)
   ...
   */
  static saveResult(result: ClaudeResult): void {
   ...
   */
  static getResult(promptId: string): ClaudeResult | null {
   ...
   */
  static getAllResults(): Map<string, ClaudeResult> {
  ...
  static getRemainingRequests(): number {
    return Math.max(0, CLAUDE_CONFIG.MAX_REQUESTS - this.getRequestsUsed());
  }
  ...
  /**
   * Auto-import batch results from static JSON if localStorage is empty.
   * Fetches from /scripts/claude-batch-results.json and populates localStorage.
   * Returns true if data was imported (caller should re-read stats).
   */
  static async autoImportFromBatch(): Promise<boolean> {
    if (typeof window === 'undefined') return false;

    // Skip if data already exists in localStorage
    const existing = localStorage.getItem(this.STORAGE_KEY);
    if (existing) return false;

    try {
      const response = await fetch('/scripts/claude-batch-results.json');
      if (!response.ok) return false;

      const data = await response.json();
      if (!data.results || !Array.isArray(data.results)) return false;

      // Import all results as a Map (the format ClaudeService expects)
      const resultsMap = new Map<string, ClaudeResult>();
      for (const result of data.results) {
        resultsMap.set(result.promptId, result);
      }

      // Store as serialized Map entries
      const serialized = JSON.stringify(Array.from(resultsMap.entries()));
      localStorage.setItem(this.STORAGE_KEY, serialized);

      // Update request counter
      localStorage.setItem(
        this.COUNTER_KEY,
        (data.metadata?.requestsUsed ?? data.results.length).toString()
      );

      console.log(`[Claude] Auto-imported ${data.results.length} batch results`);
      return true;
    } catch (error) {
      console.warn('[Claude] Auto-import failed:', error);
      return false;
    }
  }
