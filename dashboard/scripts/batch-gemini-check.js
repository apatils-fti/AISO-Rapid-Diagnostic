/**
 * Batch Gemini 2.5 Flash Check Script
 *
 * Processes 250 prompts through Gemini API
 * Results stored in format compatible with GeminiService
 */
...
function parseArgs() {
  ...
  const parsed = {
    ...
    output: './gemini-batch-results.json',
    clientId: null,
    libraryId: null,
  };
...
async function initSupabase() {
  ...
  try {
    ...
    const { data, error } = await supabase.from('runs').insert({
      ...
      mention_count: 0,
      mention_rate: 0,
      metadata: { script: 'batch-gemini-check.js', startedAt: new Date().toISOString() },
    }).select('id').single();
...
// Configuration
const GEMINI_ENDPOINT = 'http://localhost:3000/api/gemini'; // Use API route for rate limiting
const DELAY_MS = 7500; // 7.5 seconds between requests (slightly over 7s for safety)

// Load 250 prompts (same sample as Google AI Overviews)
...
/**
 * Check single prompt with Gemini
 */
async function checkPrompt(promptData, index, total) {
  ...
  try {
    // Call API route
    requestCount++;
    const data = await makeRequest(GEMINI_ENDPOINT, {
      prompt: promptText,
      promptId,
      topicId,
    });
...
async function main() {
  ...
  console.log('Gemini 2.5 Flash Batch Check');
  console.log('='.repeat(70));
