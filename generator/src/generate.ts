...
function findHighestWeightCell(
  ...
  for (const intent of INTENT_STAGES) {
    for (const iso of ISOTOPES) {
      const w = template.weights.intents[intent] * template.weights.isotopes[iso];
      // Bias toward cells that already have the most (we're filling the densest regions).
...
export function generate(opts: GenerateOptions): GenerationResult {
  ...
  // Distribute cell counts across topics. For each cell, spread its count
  // across topics whose primaryIntent matches when possible; otherwise
  // round-robin across all topics.
  for (const intent of INTENT_STAGES) {
    for (const iso of ISOTOPES) {
      ...
      const topicPool: Topic[] =
        matchingTopics.length > 0 ? matchingTopics : tagged;
